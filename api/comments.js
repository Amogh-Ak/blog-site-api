...
router.get("/:postId", async (req, res) => {
  try {
    ...
    const popularComments = post.comments
      .slice()
...
router.get("/:postId/:commentId", async (req, res) => {
  try {
    if (req.params.postId) {
      ...
      let post = await Comment.findOne({ post: req.params.postId }).populate(
...
// @route   PUT /api/comments/dislike/:postId/:commentId
// @desc    disLike or undislike a comment
router.put("/dislike/:postId/:commentId", auth, async (req, res) => {
  try {
    let post = await Comment.findOne({ post: req.params.postId });
    if (!post) {
      return res.status(404).json({ msg: "Post not found" });
    ...
    if (isdisLiked) {
      ...
      // Like the comment
      comment.dislikes.unshift({ user: req.userId });
      post = await post.save();
...
router.put("/pin/:postId/:commentId", auth, async (req, res) => {
  try {
    ...
    const isPinned =
      comment.pinned_comments.filter(
        (pin) => pin.commentId === req.params.commentId
      ).length > 0;
...
// @route   PUT /api/comments/dislike/:postId/:commentId/:replyId
// @desc    disLike or un-dislike a reply
router.put("/dislike/:postId/:commentId/:replyId", auth, async (req, res) => {
  try {
    let post = await Comment.findOne({ post: req.params.postId });
    if (!post) {
      return res.status(404).json({ msg: "Post not found" });
    ...
    const isdisLiked =
      reply.dislikes.filter((dislike) => dislike.user.toString() === req.userId)
        .length > 0;

    if (isdisLiked) {
      // Unlike the reply if already liked
      const index = reply.dislikes.findIndex(
        (dislike) => dislike.user.toString() === req.userId
      );
      ...
      // Like the reply
      reply.dislikes.unshift({ user: req.userId });
      post = await post.save();
